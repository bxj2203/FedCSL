...
for client in clients:
    ...
    for nii_seg in tqdm(nii_seg_list):
        print(nii_seg)
        nii_path = os.path.join(folder, nii_seg[:8]+'_0000.nii.gz')
        ...
        if len(has_label) == 0:
            continue
        has_label.append(has_label[0]-1)
        has_label.append(has_label[-1]-1)
        ...
        print(os.path.join(tar_folder_path,nii_seg[:8]+'.nii.gz').replace('path','fff'))
        sitk.WriteImage(sitk.GetImageFromArray(image_v3),os.path.join(tar_folder_path,nii_seg[:8]+'.nii.gz').replace('path','fff'))
        sitk.WriteImage(sitk.GetImageFromArray(label_vol),
                        os.path.join(tar_folder_path, nii_seg[:8] + '_segmentation.nii.gz').replace('path', 'fff'))
