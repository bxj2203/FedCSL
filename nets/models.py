...
class Dynamic_conv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, ratio=0.25, stride=1, padding=0, dilation=1, groups=1,
                 bias=False, K=4, temperature=30, init_weight=True):
        ...
        self.weight = nn.Parameter(torch.Tensor(K, in_planes, in_planes // self.groups, kernel_size, kernel_size),
                                   requires_grad=True)
    ...
    def forward(self, x):  # 将batch视作维度变量，进行组卷积，因为组卷积的权重是不同的，动态卷积的权重也是不同的#2 3 512 512
        ...
        aggregate_weight = torch.mm(softmax_attention, weight).view(-1, self.in_planes // self.groups, self.kernel_size,
                                                                    self.kernel_size)
...
class DYBAC(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride, padding):
        ...
        self.dybac = nn.Sequential(
            ...
            Dynamic_conv2d(in_planes=in_planes, out_planes=out_planes, kernel_size=kernel_size, stride=stride,
                           padding=padding),
...
class PyramidPooling(nn.Module):
    def __init__(self, in_channels, pool_sizes):
        ...
        self.convs = nn.ModuleList([
            nn.Conv2d(in_channels, out_channels, kernel_size=1) for _ in pool_sizes
        ])

    def forward(self, x):
        features = [x]
        for pool_size, conv in zip(self.pool_sizes, self.convs):
            avg_pool = nn.AdaptiveAvgPool2d(output_size=pool_size)
            pooled = avg_pool(x)
            convolved = conv(pooled)
            upsampled = F.interpolate(convolved, size=x.shape[-2:], mode='bilinear', align_corners=True)
            features.append(upsampled)
        return torch.cat(features, dim=1)
class UNet(nn.Module):
    def __init__(self, in_ch, out_ch):
        super(UNet, self).__init__()
        self.pyramid_pooling = PyramidPooling(in_ch, [1, 2,3])
        self.conv1 = DoubleConv1(6, 32)  # 1
        self.pool1 = nn.MaxPool2d(2)
        self.drop1 = nn.Dropout2d(0.1)
        self.conv2 = DoubleConv(32, 64)  # 2
        ...
        self.conv6 = DoubleConv(768, 384)
        self.drop6 = nn.Dropout2d(0.2)

        self.conv7 = DoubleConv(512, 192)
        self.drop7 = nn.Dropout2d(0.2)

        self.conv8 = DoubleConv(256, 128)
        self.drop8 = nn.Dropout2d(0.1)

        self.conv9 = DoubleConv(160, 32)
