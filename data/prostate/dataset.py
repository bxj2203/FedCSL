...
class Prostate(Dataset):
    ...
    def __init__(self, site, base_path=None, split='train', transform=None):
        channels = {'client1':3, 'client2':3,'client3':3}
        assert site in list(channels.keys())
        self.split = split

        base_path = base_path if base_path is not None else r'path'
...
#            a = np.load(os.path.join(base_path,site,f_name))
            ...
            images.append(np.load(os.path.join(base_path,site,f_name)).astype(np.int16))
